...
class sample_relaxation_net(torch.nn.Module):

    def __init__(self, temp, function, scale_param):
        super(sample_relaxation_net, self).__init__()
        # an affine operation: y = Wx + b
        self.sigmoid = torch.nn.Sigmoid()
        self.function = function
    ...
    def forward(self, u, params):
        inputs = torch.cat((u,params), 0).unsqueeze(1)
        out = self.sigmoid(self.L(inputs))
        return out
